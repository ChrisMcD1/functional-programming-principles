...
object Anagrams extends AnagramsInterface:
  ...
  extension (occurrences: Occurrences)
    def toOccurences: Occurrences =
      occurrences.filter((_, count) => count > 0).sortBy((char, _) => char)
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.groupMapReduce(_.toLower)(_ => 1)(_ + _).toList.toOccurences
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    def generateAllSets(occurrences: Occurrences): Set[Occurrences] =
      val newOccurrences =
        occurrences
          .map((char, count) =>
            (char, count - 1) :: occurrences.filterNot((otherChar, _) =>
              otherChar == char
            )
          )
          .map(_.toOccurences)

      Set(occurrences) ++ newOccurrences.flatMap(generateAllSets(_))
    generateAllSets(occurrences).toList
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    x.map((xChar, xCount) =>
      y.find((yChar, _) => yChar == xChar) match
        case Some((_, yCount)) => (xChar, xCount - yCount)
        case None              => (xChar, xCount)
    ).toOccurences
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =
    def getWordsFromOccurances(
        occurrences: Occurrences
    ): Option[List[Word]] =
      if (occurrences.isEmpty) then Some(List())
      else dictionaryByOccurrences.get(occurrences)

    def concatenateSentenceWithPossibleEndings(
        prefix: Sentence,
        remainingOccurences: Occurrences
    ): List[Sentence] =
      getWordsFromOccurances(remainingOccurences).match
        case None => List()
        case Some(nextWords) =>
          nextWords.map(nextWord => prefix.appended(nextWord))

    def occurrencesToSentences(
        prefix: Sentence,
        occurrences: Occurrences
    ): List[Sentence] =
      occurrences match
        ...
        case _ =>
          combinations(occurrences)
            .flatMap(occurrenceSubset =>
              val newSentences =
                concatenateSentenceWithPossibleEndings(prefix, occurrenceSubset)
              val remainingOccurences =
                subtract(occurrences, occurrenceSubset)
              newSentences.flatMap(newPrefix =>
                occurrencesToSentences(newPrefix, remainingOccurences)
              )
            )

    val sentenceOccurrencesReal = sentenceOccurrences(sentence)
    occurrencesToSentences(Nil, sentenceOccurrencesReal)
