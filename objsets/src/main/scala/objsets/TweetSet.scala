...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet =
    throw java.util.NoSuchElementException("most retweeted in empty set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    val childrenAccumulated =
      left.filterAcc(p, acc).union(right.filterAcc(p, acc))
    if p(elem) then childrenAccumulated.incl(elem)
    else childrenAccumulated
  ...
  def descendingByRetweet: TweetList =
    ...
    Cons(highest, remove(highest).descendingByRetweet)
...
object Nil extends TweetList:
  ...
  def isEmpty = true
  def append(list: TweetList) = list

class Cons(val head: Tweet, val tail: TweetList) extends TweetList:
  def isEmpty = false
  def append(list: TweetList): TweetList = Cons(head, tail.append(list))
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter((tweet) =>
    google.exists((googleKeyword) => tweet.text.contains(googleKeyword))
  )

  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter((tweet) =>
    apple.exists((appleKeyword) => tweet.text.contains(appleKeyword))
  )
