...
trait Huffman extends HuffmanInterface:
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    chars.groupBy(char => char).map((char, list) => (char, list.length)).toList
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs
      .sortBy((_, weight) => weight)
      .map((char, weight) => Leaf(char, weight))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] =
    trees match
      case left :: right :: next =>
        (makeCodeTree(left, right) :: next).sortBy(weight(_))
      case head :: Nil => trees
      case Nil         => trees
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    ...
    def helper(tree: CodeTree, bits: List[Bit], acc: List[Char]): List[Char] =
      def treeWalker(subtree: CodeTree, bits: List[Bit]): (List[Bit], Char) =
        subtree match
            ...
            bits match
              case Nil =>
                throw new NoSuchElementException(
                  "Tried to walk the tree at a fork with no bits!"
                )
              case head :: next =>
                head match
                  case 0 => treeWalker(left, next)
                  case 1 => treeWalker(right, next)
      bits match
        case Nil => acc
        case _ =>
          val (nextBits, newChar) = treeWalker(tree, bits)
          helper(tree, nextBits, acc ::: List(newChar))
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] =
    def encodeChar(tree: CodeTree, char: Char): List[Bit] =
      @tailrec
      def encodeCharWalker(
          subTree: CodeTree,
          char: Char,
          acc: List[Bit]
      ): List[Bit] =
        def treeContainsChar(subSubTree: CodeTree, char: Char) =
          subSubTree match
            case Fork(_, _, chars, _) => chars.contains(char)
            case Leaf(leafChar, _)    => leafChar == char
        subTree match
          case Fork(left, right, chars, _) =>
            if treeContainsChar(left, char) then
              encodeCharWalker(left, char, acc.appended(0))
            else encodeCharWalker(right, char, acc.appended(1))
          case Leaf(_, _) => acc
      encodeCharWalker(tree, char, Nil)
  ...
  def convert(tree: CodeTree): CodeTable =
    tree match
      case Fork(left, right, chars, weight) =>
        mergeCodeTables(
          convert(left).map((char, bits) => (char, 0 :: bits)),
          convert(right).map((char, bits) => (char, 1 :: bits))
        )
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] =
    ...
    def quickCharToBits(table: CodeTable, char: Char): List[Bit] =
      table.find((tableChar, _) => tableChar == char) match
        case Some(value) => value._2
        case None        => throw new Error("Could not find char in table")
    text.flatMap(quickCharToBits(table, _))

object Huffman extends Huffman

@main def hello() = println(Huffman.decodedSecret.mkString)
