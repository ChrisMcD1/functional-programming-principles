...
class FunSetSuite extends munit.FunSuite:
  ...
  test("exists works for 1") {
    new TestSets:
      assert(exists(s1, (num: Int) => num == 1), "Union 1")
  }

  test("exists returns false for 0") {
    new TestSets:
      assert(!exists(s1, (num: Int) => num == 0), "Union 1")
  }

  test("basic forall") {
    new TestSets:
      assert(forall(s1, (num: Int) => num.abs < 10000), "Union 1")
  }
  test("basic map") {
    new TestSets:
      var s = union(union(s1, s2), s3)
      assert(contains(map(s, x => x * 2), 6), "Map1")
  }
